...
Registers a combo list (combo box) widget that allows selecting a single item from a list of strings. Returns a boolean indicating the success of the item creation.

```lua
add_single_combo(string: name, string: key, number?: default, table?: items, boolean?: safe): boolean
```

<ParamField path="name" type="string" required>The text label displayed next to the combo list in the menu.</ParamField>
...
<ParamField path="default" type="number">The **initial 0-based index** of the item that should be selected when the script loads. Defaults to `0` (the first item).</ParamField>
