...
Gets the current process ID. Returns the process ID as a number.
...
Gets the delta time (time elapsed since the last frame). Returns the delta time in seconds as a number.
