...
This section lists all available C++ functions exposed within the global **`memory`** table. These methods allow for low-level reading and writing of specific data types directly to and from process memory addresses.
...
<ParamField path="address" type="number" required>The memory address (a number) to which to write the value.</ParamField>
<ParamField path="value" type="number" required>The unsigned 32-bit integer value to write.</ParamField>
